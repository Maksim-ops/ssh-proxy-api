...
router = APIRouter(prefix="/api/v1/admin/servers", tags=["crud-servers"])
...
async def get_server_by_id(server_id: int, user: AuthenticatedUser = Depends(require_auth)):
    require_superadmin(user)
    try:
...
async def create_server_endpoint(payload: ServerCreate, user: AuthenticatedUser = Depends(require_auth)):
    require_superadmin(user)
    try:
...
async def update_server_endpoint(server_id: int, payload: ServerUpdate, user: AuthenticatedUser = Depends(require_auth)):
    require_superadmin(user)
    try:
...
async def delete_server_endpoint(server_id: int, user: AuthenticatedUser = Depends(require_auth)):
    require_superadmin(user)
    try:
